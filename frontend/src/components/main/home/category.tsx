...
const MainCategory = (props: Props) => {
    ...
    return (
        <div className='border-y-2 border-primary mt-40xs md:mt-40md'>
            <div className='mt-20xs md:mt-20md text-23xs md:text-23md leading-27xs md:leading-27md font-bold'>DANH MỤC</div>
            <div className='mt-20xs md:mt-20md mb-20xs md:mb-20md grid grid-cols-3 md:grid-cols-5 gap-50md'>
                {
                    user?.grade
                        ? <>{subjects && subjects.map((value: SubjectType) => (
                            <div key={value.id} className='flex justify-center flex-col text-center'>
                                <div className='relative'>
                                    ...
                                    <img src={value.icon} className='!w-66xs md:!w-66md !h-66xs md:!h-66md !absolute !top-[50%] !left-[50%] !translate-y-[-50%] !translate-x-[-50%] !z-10' />
                                </div>
                                <div><Link href={`/subject/${value.slug}`}>{value.name}</Link></div>
                            </div>
                        ))}</>
                        : <></>
                }
