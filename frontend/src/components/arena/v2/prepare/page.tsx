...
const PrepareArena: React.FC<Props> = ({ arena, handleStart, user }) => {
    return (
        <div><div className='p-20xs md:p-20md'>
            <div className='flex justify-between items-center'>
                ...
                <div className='flex items-center gap-7xs md:gap-7md'>
                    ...
                    <div>
                        <div className='font-bold text-primary'><Countdown onComplete={handleStart} date={DateTime.fromFormat(arena.start_at, 'yyyy-MM-dd HH:mm:ss').toMillis()} /></div>
                    </div>
                </div>
            </div>
            <div className="grid grid-cols grid-cols-1 md:grid-cols-3 gap-10xs md:gap-10md w-full mt-20xs md:mt-20md max-h-340md overflow-y-auto border-black border-y py-10md">
                ...
                {arena.joined && arena.joined.map((user, index) => {
                    return (
                        <div className='group/item border border-black py-24xs md:py-24md flex justify-between rounded hover:bg-primary hover:text-white' key={user.id}>
