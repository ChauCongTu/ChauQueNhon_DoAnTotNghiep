...
const TopicAdmin = (props: Props) => {
    ...
    const fetch = async (page?: number) => {
        if (page) {
            const res = await getTopics({ perPage: 12, page: page });
            ...
            const res = await getTopics({ perPage: 12 });
            if (res.status.success) {
                setCurrent(res.data[0].current_page);
                setTotal(res.data[0].total);
                setTopics(res.data[0].data);
    ...
    return (
        <div className='shadow'>
            <div className='bg-white rounded mr-5xs md:mr-5md mt-5xs md:mt-5md'>
                <div className='px-10xs md:px-10md text-24xs md:text-24md font-bold border-b py-20xs md:py-20md'>Quản lý chủ đề thảo luận</div>
                <div className='px-10xs md:px-10md py-10xs md:py-10md'>
                    {
                        user
                            ? <>
                                ...
                                {
                                    topics && <TopicTable topics={topics} setTopics={setTopics} page={current} fetch={fetch} />
                                }
